Initial Setup:-
A blank solution was created called RGC2020MVC, with 'Create new Git Repository' checked, ignoring Visual Studio type files.
...
the GitHub desktop application. In this application, a new repository was ADDED, and the the solution folder given as the 
location of the .gitattributes and .gitignore files. This procedure added the local repository to the list of repositories in 
the application. Whe selecting the 'Changes' button, all of the non-committed files, ignoring the Visual Studio type files,
were shown. A name of 'Initial Commit' was given to these changes, and then committed. Once committed, the 'Publish' button was 
clicked to publish these files to GitHub.

To get the files onto another computer, load up the windows GitHub application, and select Clone a Repository. This will
show all the online repositories that are available to clone for this GitHub user. When asked where to clone to, select the parent folder 
e.g C:\Users\SomeUSer\Documents\Visual Studio 2015\Projects
A new solution folder will be created in the parent folder.
...
To output the Database Schema and Data to a SQL script, which contains all migrations, you can use:-
Update-Database -script
If you want to output the database to a script from a certain migration, you can use:-
Update-Database -script -sourceMigration: SomeMigrationName


To find out which migrations have been applied to a database, do as follows from the package manager console:-
get-migrations
You can also check the __MigrationHistory table in the database, which will hold all the migrations run.
...
This will only show the Login Page as the main page, and require a valid login to proceed. Normally, a small portion of
